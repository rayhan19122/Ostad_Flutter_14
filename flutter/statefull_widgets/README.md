...
def weekday_from_date(day, month, year):
    ...
    # Determine leap year and set Jan/Feb anchors
    ...
    # Month anchor dates (index 0 unused, 1-12 for Jan-Dec)
    month_anchor = [
        0,
        jan_anchor, feb_anchor, 14, 4, 9, 6, 11, 8, 5, 10, 7, 12
    ]
    
    # Calculate century anchor
    century = math.floor(year / 100)
    anchor = (5 * (century % 4) + 2) % 7
    
    # Calculate year doomsday
    y = year % 100
    a = math.floor(y / 12)
    b = y % 12
    c = math.floor(b / 4)
    ...
    # Calculate weekday from doomsday
    reference_day = month_anchor[month]
    diff = day - reference_day
...
# Example usage
print(weekday_from_date(16, 1, 2026))  # Output: Friday
print(weekday_from_date(25, 12, 2025)) # Output: Thursday
print(weekday_from_date(1, 1, 2000))   # Output: Saturday
```
...
function weekdayFromDate(day, month, year) {
    ...
    const doomsday = (anchor + a + b + c) % 7;
    
    // Calculate weekday from doomsday
    const referenceDay = monthAnchor[month];
    const diff = day - referenceDay;
    let weekdayIndex = (doomsday + diff) % 7;
...
// Example usage
console.log(weekdayFromDate(16, 1, 2026));  // Output: Friday
console.log(weekdayFromDate(25, 12, 2025)); // Output: Thursday
console.log(weekdayFromDate(1, 1, 2000));   // Output: Saturday
```
...
string weekdayFromDate(int day, int month, int year) {
    ...
    // Determine leap year and set Jan/Feb anchors
    int janAnchor = isLeapYear(year) ? 4 : 3;
    int febAnchor = isLeapYear(year) ? 29 : 28;
    ...
    int b = y % 12;
    int c = floor(b / 4);
    int doomsday = (anchor + a + b + c) % 7;
    
    // Calculate weekday from doomsday
    int referenceDay = monthAnchor[month];
    int diff = day - referenceDay;
    int weekdayIndex = (doomsday + diff) % 7;
...
int main() {
    cout << weekdayFromDate(16, 1, 2026) << endl;  // Output: Friday
    cout << weekdayFromDate(25, 12, 2025) << endl; // Output: Thursday
    cout << weekdayFromDate(1, 1, 2000) << endl;   // Output: Saturday
    return 0;
}
```
